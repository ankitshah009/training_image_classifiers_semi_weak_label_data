...
for trial in {0..4}
do
  for count_loss_weight in 1
  do
    t=$trial
    echo "Run Resue Bag dataset"
    CUDA_VISIBLE_DEVICES=$CUDA_DEVICES python semi_weak_main.py \
      --comment "reg=$regularize.loss=$loss_type.n=$nsample.bag=$bagsize.trial=$t.beta=$beta.lr=$lr.bs=$batchsize.ew=$entropy_weighted.blw=$binary_loss_weight.elw=$entropy_loss_weight.clw=$count_loss_weight" \

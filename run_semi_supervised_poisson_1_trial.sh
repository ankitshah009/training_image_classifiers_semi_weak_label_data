...
do
  ...
  --comment "poisson.reg=$regularize.loss=$loss_type.n=$nsample.bag=$bagsize.trial=$t.beta=$beta.lr=$lr.bs=$batchsize.ew=$entropy_weighted.blw=$binary_loss_weight.elw=$entropy_loss_weight.clw=$count_loss_weight.reuse=$reuse.classifier_type=$base_classifier" \
  --meta-path="./meta/reuse=$reuse.n=$nsample.bag=$bagsize.beta=$beta.poisson" \
  -bs $batchsize \
  -e $epoch \
  -t=$t \
  --loss-type $loss_type \
...
#do
#  t=$trial
#  echo "Evalueate Resue Bag dataset"
#  CUDA_VISIBLE_DEVICES=$CUDA_DEVICES python semi_weak_main.py \
#  --comment "eval.weighted=$entropy_weighted.poisson.loss=$loss_type.n=$nsample.bag=$bagsize.trial=$t.beta=$beta" \
#  --meta-path "./meta/reuse=$reuse.n=$nsample.bag=$bagsize.beta=$beta.poisson" \
#  -bs $batchsize \
#  -e $epoch \
#  -t=$t \
